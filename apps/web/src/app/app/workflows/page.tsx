...
export default function WorkflowsPage() {
  const [templates, setTemplates] = useState<{ id: string; name: string; steps: string[]; description: string }[]>([]);
      ...
      const run = await api<{ output_payload: Record<string, unknown>; status: string }>(
        "/workflows/launch",
        {
          ...
          body: JSON.stringify({
            ...
            workflow,
            goal: "Launch DigiSEO AI product campaign",
          }),
        },
      );
      setResult(run.output_payload);
      setMsg(`Workflow ${run.status}`);
    } catch (err) {
  ...
  return (
    <div>
      <h1 className="font-display text-3xl">Multi-agent workflows</h1>
      <p className="mt-2 text-muted">
        Orchestrate keyword → content → AEO → social → analytics in one run.
      </p>
      ...
      <h2 className="mt-12 font-display text-xl">Phase 3 agents</h2>
        ...
        {[
          ...
          <button
            ...
            onClick={async () => {
              ...
              const path =
                agent === "backlink"
                  ? `/outreach/discover?workspace_id=${session.workspace_id}`
                  : agent === "ppc"
                    ? "/ppc/optimize"
                    : "/local-seo/optimize";
              const body =
                agent === "backlink"
                  ...
                  : {
                      workspace_id: session.workspace_id,
                      ...(agent === "local_seo"
                        ? { business_name: "Demo Local Biz" }
                        : { goal: "Lead gen", platform: "google" }),
                    };
